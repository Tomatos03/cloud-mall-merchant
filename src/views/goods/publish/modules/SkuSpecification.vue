            ...
            <div class="border-t border-gray-100 pt-8">
                ...
                >
                    ...
                    <template
                        ...
                        :key="spec.name"
                        #[spec.name]="{ row }: { row: GoodsSkuItem }"
                    >
                    ...
                    <!-- 库存列 -->
                    <template #inventory="{ row }: { row: GoodsSkuItem }">
                        <el-input-number
                    ...
                    <!-- 状态列 -->
                    <template #status="{ row }: { row: GoodsSkuItem }">
                        <el-switch v-model="row.status" :disabled="isReadonly" />
                    </template>
                </Table>
            </div>
        </div>
...
<script setup lang="ts">
    ...
    import type { GoodsSpecification, GoodsSkuItem, GoodsSkuSpec } from '@/api/goods'
    ...
    const skuColumns = computed(() => {
        const specs = internalSpecifications.value
            .filter((s) => s.name)
            .map((s) => ({
                id: s.name,
                label: s.name,
        ...
        return [
            ...specs,
            { id: 'price', label: '价格', key: 'price', width: 180, align: 'center' as const },
            {
                id: 'inventory',
                label: '库存',
                key: 'inventory',
